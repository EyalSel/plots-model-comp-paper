...
if __name__ == "__main__":
    ...
    cost = get_cpu_cost(cloud, 8) + get_gpu_cost(cloud, "v100", num_gpus=4)
    slo = 0.5
    lam = 58
    cv = 4.0
    ...
    if result == None:
        print "================"
        print "Could not find a configuration for the given inputs"
        print "================"
        import sys
        sys.exit(1)
    node_configs, perfs, response_time = result
    config_obj = Configuration(slo, cost, lam, cv, node_configs, perfs,
                               response_time, 1.0) # 1.0 for utililization
    results_fname = "aws_image_driver_one_ifl_configs_slo_{slo}_cv_{cv}.json".format(slo=slo,cv=cv)
    with open(results_fname, "w") as f:
        json.dump([config_obj.__dict__], f, indent=4)